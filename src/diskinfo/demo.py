...
def disklist_demo():
    ...
    panel = Panel(
        f'[markdown.strong]There {verb} [bold sky_blue2]{disk_num}[/] disk{plural} installed in this'
...
def disk_demo(name: str):
    ...
    d = Disk(name)
    panel = Panel(
        f'[markdown.strong]Standard disk attributes of:[/] [bold green]{name}[/]',
        box=box.MINIMAL,
        expand=False,
    )
    table = Table(border_style='gray30', box=box.MINIMAL)
    ...
    table.add_row(
        'physical block size',
        f'[bold wheat4]{str(d.get_physical_block_size())} bytes [/]',
    )
    ...
    rprint(
        Panel(
            group,
            title='diskinfo demo: disk attributes',
            title_align='left',
            border_style='gray30',
            expand=False,
        )
...
def smart_demo(name: str):
    ...
    if not sd:
        panel = Panel(
            f'[markdown.strong]SMART attributes of:[/] [bold green]{name}[/]\n'
            '\n'
    ...
    rprint(
        Panel(
            group,
            title='diskinfo demo: SMART attributes',
            title_align='left',
            border_style='gray30',
            expand=False,
        )
...
def partition_demo(name: str):
    ...
    panel = Panel(
        f'[markdown.strong]There are {len(plist)} partitions on disk[/] [bold green]{name}[/]\n'
    ...
    for p in plist:
        free_size_str = (
            f'{round(float(p.get_fs_free_size() / p.get_part_size() * 100)):>3d} %'
        )
        table.add_row(
            p.get_name(),
            p.get_fs_type(),
            str(p.get_part_offset()),
            str(p.get_part_size()),
            p.get_fs_label(),
            p.get_fs_mounting_point(),
            free_size_str,
        )
    group = Group(panel, table)
    rprint(
        Panel(
            group,
            title='[markdown.strong]diskinfo demo: partitions[/]',
            title_align='left',
            border_style='gray30',
            expand=False,
        )
