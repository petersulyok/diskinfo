...
def disklist_demo():
    ...
    panel = Panel(f"[markdown.strong]There {verb} [bold sky_blue2]{disk_num}[/] disk{plural} installed in this"
        ...
        f" SSD(s), [bold sky_blue2]{nvme_num}[/] NVME(s)[/]",
    ...
    for d in disks:
        ...
        if temp:
            ...
            temp_str = ""
        table.add_row(d.get_name(), d.get_type_str(), d.get_model(), d.get_path(), temp_str,
...
def disk_demo(name: str):
    ...
    d = Disk(name)
    panel = Panel(f"[markdown.strong]Standard disk attributes of:[/] [bold green]{name}[/]", box=box.MINIMAL,
                  expand=False)
    table = Table(border_style="gray30", box=box.MINIMAL)
    ...
    group = Group(panel, table)
    rprint(Panel(group, title="diskinfo demo: disk attributes", title_align="left", border_style="gray30", expand=False))
...
def smart_demo(name: str):
    ...
    if not sd:
        panel = Panel(f"[markdown.strong]SMART attributes of:[/] [bold green]{name}[/]\n"
                      ...
                      "[bold red]or 'smartctl' command cannot be executed![/]", box=box.MINIMAL, expand=False)
        group = Group(panel)

    else:
        panel = Panel(f"[markdown.strong]SMART attributes of:[/] [bold green]{name}[/]", box=box.MINIMAL,
                      expand=False)
        table = Table(border_style="gray30", box=box.MINIMAL)
        table.add_column("Attribute", justify="left", style="steel_blue1")
        ...
        if d.is_nvme():
            if hasattr(sd.nvme_attributes, "critical_warning"):
                table.add_row("critical warning", f"[bold green]{str(sd.nvme_attributes.critical_warning)}[/]"
                              if sd.nvme_attributes.critical_warning == 0
                              else f"[bold red]{str(sd.nvme_attributes.critical_warning)}[/]")
...
def partition_demo(name: str):
    ...
    # Display raw filesystem information if the disk has no partition table.
    if d.has_filesystem():

        fs = d.get_filesystem()
        fs_table = Table(border_style="gray30", box=box.MINIMAL)
        fs_table.add_column("Attribute", justify="left", style="steel_blue1")
        fs_table.add_column("Value", justify="left", style="bold orchid")
        fs_table.add_row("type", f"[bold orange1]{fs.get_fs_type()}[/]")
        fs_table.add_row("label", f"[bold purple3]{fs.get_fs_label()}[/]")
        fs_table.add_row("uuid", f"[bold green]{fs.get_fs_uuid()}[/]")
        ...
        fs_panel = Panel(f"[markdown.strong]Raw filesystem on disk[/] [bold green]{name}[/]", box=box.MINIMAL,
                         expand=False)
        group = Group(fs_panel, fs_table)

    # Display partition list of the disk.
    else:
        ...
        for p in plist:
            fs = p.get_filesystem()
            if p.get_part_size() > 0:
                free_size_str = f"{round(float(fs.get_fs_free_size() / p.get_part_size() * 100)):>3d} %"
            else:
                free_size_str = "  - %"
            table.add_row(p.get_name(), fs.get_fs_type(), str(p.get_part_offset()), str(p.get_part_size()),
                          fs.get_fs_label(), fs.get_fs_mounting_point(), free_size_str)
            group = Group(panel, table)
...
def usage():
    ...
    print(
        "Usage: python -m diskinfo.demo [device] [-s][-p]\n"
        "Examples:\n"
