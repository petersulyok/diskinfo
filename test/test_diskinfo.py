...
def test_diskinfo_init_discovers_disks():
    ...
    with (patch("diskinfo.diskinfo.Context", return_value=mock_ctx),
...
def test_get_disk_number_included_and_excluded_overlap_raises_valueerror():
    ...
    di = _make_diskinfo([_make_disk("sda", DiskType.SSD)])
    with pytest.raises(ValueError):
        di.get_disk_number(included={DiskType.SSD, DiskType.HDD}, excluded={DiskType.HDD})
    with pytest.raises(ValueError):
        di.get_disk_number(included={DiskType.HDD, DiskType.SSD}, excluded={DiskType.SSD})
