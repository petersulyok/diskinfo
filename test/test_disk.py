...
def test_init_by_disk_name(fixture_name, expected_type, request):
    ...
    exp_phys_bs = mock_dev.attributes.asint("queue/physical_block_size")
    exp_log_bs = mock_dev.attributes.asint("queue/logical_block_size")
    assert d.get_physical_block_size() == exp_phys_bs
    assert d.get_logical_block_size() == exp_log_bs
...
def test_init_unknown_disk_name_raises_valueerror():
    ...
    with (patch("diskinfo.disk.Context", return_value=mock_ctx),
          patch("diskinfo.disk.Devices") as mock_devs):
        mock_devs.from_name.side_effect = DeviceNotFoundAtPathError("/sys/class/block/nonexistent")
        with pytest.raises(ValueError):
            Disk("nonexistent")
...
@pytest.mark.parametrize("fixture_name", ["ssd_device", "hdd_device"])
def test_init_by_wwn(fixture_name, request):
    """Disk(wwn=...) locates the disk by scanning udev ID_WWN properties."""
    mock_dev = request.getfixturevalue(fixture_name)
    wwn = mock_dev.get("ID_WWN")

    with pyudev_patched(mock_dev):
        d = Disk(wwn=wwn)
    assert d.get_name() == mock_dev.sys_name
    assert d.get_wwn() == wwn


def test_init_invalid_wwn_raises_valueerror(ssd_device):
    """Disk(wwn=...) raises ValueError when no device matches."""
    with pyudev_patched(ssd_device):
        with pytest.raises(ValueError):
            Disk(wwn="0xdeadbeefdeadbeef")
...
def test_get_temperature_reads_hwmon_sysfs_file():
    ...
    d = Disk.__new__(Disk)
    with tempfile.NamedTemporaryFile(mode="w", suffix="_temp1_input", delete=False) as f:
        f.write("43000")
        hwmon_file = f.name
    try:
...
def _make_legacy_attr(num, name, flags, value_int, worst, thresh, _type, updated, when_failed, raw_int):
    """Return a single MagicMock representing a pySMART legacy SMART attribute."""
    a = MagicMock()
    a.num = num
    a.name = name
    a.flags = flags
...
def _make_hdd_legacy_attrs() -> list:
    ...
    return [
        _make_legacy_attr(5, "Reallocated_Sector_Ct", 0x0033, 100, 100, 10, "Pre-fail", "Always", "-", 0),
        _make_legacy_attr(9, "Power_On_Hours", 0x0032, 95, 95, 0, "Old_age", "Always", "-", 23268),
        _make_legacy_attr(12, "Power_Cycle_Count", 0x0032, 92, 92, 0, "Old_age", "Always", "-", 7103),
        _make_legacy_attr(190, "Airflow_Temperature_Cel", 0x0032, 72, 45, 0, "Old_age", "Always", "-", 28),
    ]
...
def test_get_smart_data_nvme():
    ...
    with (patch.object(pySMART.SMARTCTL, "info", return_value=_SMART_INFO_OK),
...
def test_repr_contains_all_attributes(ssd_device):
    ...
    assert "filesystem=None" in result


# ── additional branch-coverage tests ─────────────────────────────────────────


def test_init_wwn_no_match_when_device_has_no_wwn():
    """Disk(wwn=...) raises ValueError when the only device has no ID_WWN property."""
    dev = _make_device(
        name="sda",
        dev_id="8:0",
        size=2000409264,
...
def test_init_byid_name_device_not_found_raises_valueerror():
    ...
    with (patch("diskinfo.disk.Context", return_value=mock_ctx),
        ...
        mock_devs.from_name.side_effect = DeviceNotFoundAtPathError("/sys/class/block/sda")
        with pytest.raises(ValueError):
...
# ── raw filesystem detection ──────────────────────────────────────────────────


def test_disk_with_raw_filesystem():
    """Disk detects a raw filesystem when no partitions and no partition table exist."""
    dev = _make_device(
        name="sdb",
        dev_id="8:16",
        size=2097152,
    ...
    assert d.has_filesystem() is True
    fs = d.get_filesystem()
    assert fs is not None
    assert fs.get_fs_type() == "ext4"


def test_disk_without_raw_filesystem():
    """Disk does not detect a raw filesystem when a partition table exists."""
    dev = _make_device(
        name="sda",
        dev_id="8:0",
        size=2000409264,
        ...
        log_bs=512,
        rotational="0",
        model="Partitioned Disk",
        part_table_type="gpt",
        part_table_uuid=str(uuid.uuid4()),
        fs_type="ext4",  # stale udev property
    )
    with pyudev_patched(dev):
        d = Disk(dev.sys_name)

    assert d.has_filesystem() is False
    assert d.get_filesystem() is None


def test_disk_blank_no_filesystem():
    """Disk does not detect a raw filesystem on a blank disk (no partitions, no fs_type)."""
    dev = _make_device(
        name="sdc",
        dev_id="8:32",
        size=2097152,
...
def test_disk_with_partitions_no_raw_filesystem(ssd_device):
    """Disk does not detect a raw filesystem when partitions exist."""
    part = _make_partition_device(
        ssd_device.attributes.asstring("dev"), 1, ssd_device.sys_name
    )
    ssd_device.children = [part]

    with (patch("builtins.open", mock_open(read_data="")),
          pyudev_patched(ssd_device)):
        d = Disk(ssd_device.sys_name)
